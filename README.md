 ...
 * The terminal clamps must support common cable strands
 * Protect all inputs against ESD damages
 * No handling of mains voltage neccessary
 * Make a board design which fits in a of the shelf case – but the casing needs to be modified
...
As microcontroller an [Arduino Nano ESP32](https://docs.arduino.cc/hardware/nano-esp32/) is used. This, or compatible models, offer sufficient resources to program even more complex control systems. It also allows the use of MicroPython, alternativly.
 ...
 * The board offers the flexibilty to use two different display sizes. Two diagonally opposite screw fixings are provided for each size
 ...
 * Each analogue inputs provide a small current voltage source to supply external cicuits.

## Todo
 * Currently temperator (Version V1.1) source all voltages to the temperature sensors directly from voltage regulator. There must be  protection circuit built in to protect the voltage regulator.
 * Sometimes DS18B20 cease to send data. It would be helful if the supply voltage to the sensors can be switched off and on again to restart the sensors.
